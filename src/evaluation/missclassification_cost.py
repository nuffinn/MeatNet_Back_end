...
def evaluate_mcc(pred, true):
    ...
    device = get_constants('device')['device']

    matrix = torch.tensor(cost_matrix['misclassification_cost_matrix']).to(device)
    pred = pred.to(device)
    true = true.to(device)
    ...
    for i in range(len(true)):
        ...
        true_i = true[i].unsqueeze(0)  # Add a dimension of size 1 to make it a row vector
        pred_i = pred[i].unsqueeze(0)  # Add a dimension of size 1 to make it a row vector
        print(true_i, pred_i)
        # Compute the product in the specified order
        product = true_i.mm(matrix).mm(pred_i.t())  # .t() is used to transpose pred_i
